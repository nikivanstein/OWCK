...
void Flame_SetMatrix( Flame *self, float *data[], int n, int m )
{
	...
	//printf( "call Flame_Clear\n" );
	Flame_Clear( self );
	self->N = n;
	self->KMAX = MAX;
	//printf( "alloc stuff with size %i\n",n );
	...
	for(i=0; i<n; i++){
		...
		if( m ==0 ){
			...
			//printf( "data is distance matrix\n" );
			for(j=0; j<n; j++){
				vals[j].index = j;
			...
			//printf( "data is raw matrix\n" );
			for(j=0; j<n; j++){
				//printf( "in loop j=%i, %i\n",j,vals[j].index );
				
				vals[j].index = j;
				//printf( "calling distfunc with %f,%f \n",data[i][0], data[j][0] );
...
void Print_Clusters(Flame *self,float *fuzzy,int size)
{
	...
	/*for( i=0; i<=self->cso_count; i++){
      if( i == self->cso_count )
	      ...
	      printf( "\nCluster %3i, with %6i members:\n", i+1, self->clusters[i].size );
	    for( j=0; j<self->clusters[i].size; j++){
	      if( j ){
	        printf( "," );
	        if( j % 10 ==0 ) printf( "\n" );
	      }
	      printf( "%5i", self->clusters[i].array[j] );
	    }
	    printf( "\n" );
	  }
	printf( "Fuzzy memberships: \n" );*/
	//fuzzy = (float*) realloc( fuzzy, size*sizeof(float));
	float **fuzzyships = self->fuzzyships;
	int C = self->cso_count+1;
	int N = self->N;
	for(i=0; i<N; i++){
		for(j=0; j<C; j++){
			//printf( "%i,%i=%f ", i,j,fuzzyships[i][j] );
			fuzzy[i*C+j] =  fuzzyships[i][j];
			//printf( "%f", fuzzy[i*C+j] );
		}
		//printf( "\n" );
	}
